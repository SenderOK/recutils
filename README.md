...
На данном датасете ускорение по сравнению с VW, которое даёт параллеллизм, отсутствует из-за менее эффективной работы со вводом (VW кеширует набор данных), а также поскольку для VW на этот раз использовалась просто регрессия. Ошибки методов без парных фичей сопоставимы, при добавлении парных фичей качество заметно улучшается.  
