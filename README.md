...
Перед началом обновляем список пакетов и устанавливаем Java Runtime Environment (если ещё нет) - это потребуется, чтобы запустить JAR-файл с утилитой, для Unix/Linux команды:
```
sudo apt-get update
